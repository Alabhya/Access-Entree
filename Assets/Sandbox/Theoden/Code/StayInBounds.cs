...
public class StayInBounds : MonoBehaviour
{
	...
	[SerializeField] private Vector3 maxVals; //The largest bounds of the level boundary
    ...
    void Update()
    {
		...
		if(	minVals.x > player.transform.position.x || maxVals.x < player.transform.position.x || 
			minVals.y > player.transform.position.y || maxVals.y < player.transform.position.y || 
			minVals.z > player.transform.position.z || maxVals.z < player.transform.position.z)
		{
			Respawn();
		}
    }
