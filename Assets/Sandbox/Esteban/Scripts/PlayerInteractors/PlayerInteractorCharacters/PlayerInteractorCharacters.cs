...
public class PlayerInteractorCharacters : MonoBehaviour
{
    ...
    #endregion
    #region Singleton
    private static PlayerInteractorCharacters _instance;

    public static PlayerInteractorCharacters Instance { get { return _instance; } }
    #endregion
        ...
        if(IsEnabled == true)
        {
            ...
            if (Physics.SphereCast(transform.position, 0.75f, transform.TransformDirection(Vector3.forward), out hit, 6, ResourceLayer))
            {
                ...
                if (hit.transform.GetComponent<Outline>().enabled == false)
                {
                    ...
                    hit.transform.GetComponent<CharacterDialogue>().Talk();
                    if (MyOutline != null)
                    {
